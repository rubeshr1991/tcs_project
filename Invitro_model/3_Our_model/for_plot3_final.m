...
hold on
yyaxis left
errorbar(t_data,HK1s_data_fit1,HK1s_se,'bo','MarkerSize',3,'CapSize',2)
yyaxis right
errorbar(t_data,RR1s_data_fit1,RR1s_se,'ro','MarkerSize',3,'CapSize',2)
load('PdtaR-RFP_data_new.mat')
load('PdtaR-RFP_se.mat')
yyaxis left
errorbar(t_data,HK1s_data,HK1s_se,'b^','MarkerSize',4)
plot(t1,(y1(:,2)+y1(:,7)+y1(:,9))/HK1s_0,'-b');
xlabel('Time (min)')
% ylabel('$$\frac{[HK_1^*]+[HK_1^*RR_1]+[HK_1^*RR_3]}{\phi_{HK} [HK_1]_0}$$','interpreter','latex')
ylabel('Normalized HK_c')
ylim([0 1])
yyaxis right
errorbar(t_data,RR1s_data,RR1s_se,'r^','MarkerSize',4)
plot(t1,(y1(:,4))/HK1s_0,'-r');
...
pos = get(h,'Position');
set(h,'PaperPositionMode','Auto','PaperUnits','inches','PaperSize',[pos(3), pos(4)])
print(h,'Fit_PdtaR2.pdf','-dpdf','-r300')
print(h,'Fit_PdtaR2.tif','-dtiff','-r600')
savefig(h,'Fit_PdtaR2.fig')
%%
% load('PdtaR-RFP_data_new.mat')
% load('PdtaR-RFP_se.mat')
load('Fit1_se.mat')
...
savefig(h,'Fit_PdtaR_no_errbar.fig')
%%
load('Fit2_se.mat')
set(groot,'defaultLineLineWidth',1)
set(groot,'defaultaxesfontsize',10)
