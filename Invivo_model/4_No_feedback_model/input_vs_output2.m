...
for ii = 1:length(II)
    Input = II(ii);
    IC(end) = Input;
    options = odeset('NonNegative',ones(1,length(IC)),'RelTol',10^-5);
    [t1,y1]=ode15s(@model_tcs_overall_new,[0 Time],IC,options,par2,k_deg_I,k_pstase); 
...
for ii = 1:length(II)
    Input = II(ii);
    IC(end) = Input;
    [t2,y2]=ode15s(@model_tcs_overall_new,[0 Time],IC,options,par2,k_deg_I,k_pstase); 
...
ylabel('Normalized maximal output (O_{max})')
xlim([10^-1 10^6]);
xticks(10.^[0 2 4 6])
set(h1,'Units','inches');
pos = get(h1,'Position');
