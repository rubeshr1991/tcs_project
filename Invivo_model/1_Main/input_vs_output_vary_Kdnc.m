...
for ii = 1:length(II)
    Input = II(ii)
    IC(end) = Input;
    [t2,y2]=ode15s(@model_tcs_overall_new_Kdnc,[0 Time],IC,[],par2,k_deg_I,kk,k_pstase); 
...
%%
figure(1)
a1 = legend({'K_{D-nc} = 10 nM','K_{D-nc} = 83 nM','K_{D-nc} = 1000 nM'},'FontSize',7,'Location','Best');
...
print(h1,['I_vs_Omax_Kdnc.pdf'],'-dpdf','-r300')
savefig(h1,'I_vs_Omax_Kdnc.fig')
figure(2)
a1 = legend({'K_{D-nc} = 10 nM','K_{D-nc} = 80 nM','K_{D-nc} = 1000 nM'},'FontSize',7,'Location','Best');
a1.ItemTokenSize = [15,10];
set(gca,'xscale','log')
% set(gca,'yscale','log')
xlabel('Input signal (I_0)')
...
print(h2,['I_vs_Otot_Kdnc.pdf'],'-dpdf','-r300')
savefig(h2,'I_vs_Otot_Kdnc.fig')